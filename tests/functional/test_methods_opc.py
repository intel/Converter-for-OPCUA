...
if __name__ == "__main__":
    ...
    try:
        ...
        for child in children:
            if "OpcPlugin" in child.get_display_name().to_string():
                obj = child
        assert obj, "The OpcPlugin Plugin is offline."

        data = obj.get_child(["2:OpcServer#0", "2:Random.Int4.Result"])
        handler = SubHandler()
        ...
        while True:
            ...
            if delay % 10 == 0:
                print('\r\ntest write')
                res = obj.call_method("2:write", "Triangle Waves.Real8", "0")
                print("method result is: ", res)

                print(
                    '\r\ntest read, it should receive the PI value since we write to 0')
                res = obj.call_method("2:read", "Triangle Waves.Real8")
                print("method result is: ", res)
            time.sleep(1)
